...
class RideAvroProducer:
    def __init__(self, props: Dict):
        ...
        key_schema_str = self.load_schema(props['schema.key']) # Load key schema
        value_schema_str = self.load_schema(props['schema.value']) # Load value schema
        schema_registry_props = {'url': props['schema_registry.url']} # Schema Registry URL
        schema_registry_client = SchemaRegistryClient(schema_registry_props) # Create Schema Registry client
        self.key_serializer = AvroSerializer(schema_registry_client, key_schema_str, ride_record_key_to_dict) # Create key serializer 
        self.value_serializer = AvroSerializer(schema_registry_client, value_schema_str, ride_record_to_dict) # Create value serializer
